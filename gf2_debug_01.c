...
p_gf2_berlekamp_nontrivial_bases_html( int bit_size, uint32_t* a, int* basis_count, uint32_t* bases )
{
  ...
  // only print rref for a_deg <= 256; need to rethink visualization for larger values
  // e.g. animated gif?
  if ( a_deg <= 256 )
  {
...
p_gf2_berlekamp_irreducible_factors_html( int bit_size, uint32_t* a, int* irreducible_factor_count, uint32_t* irreducible_factors )
{
  ...
  while (i<(*irreducible_factor_count))
  {
    ...
    html_page_push( "p_gf2_berlekamp_nontrivial_factors( %s )", fname );
...
void
p_gf2_factor_html( int bit_size, uint32_t* x, uint32_t* irreducible_factors, int* irreducible_factor_count )
{
  int       x_deg                       = p_gf2_deg( bit_size, x );
  uint32_t* sf_factors                  = p_gf2_nalloca( bit_size, x_deg );

  int*      sf_factors_pow              = (int*)__builtin_alloca( sizeof(int) * x_deg );
  uint32_t* sf_irreducible_factors      = p_gf2_nalloca( bit_size, x_deg+1 );
  int*      sf_irreducible_factor_count = (int*)__builtin_alloca( sizeof(int) * (x_deg+1) );
  int       sf_factor_count             = 0;

  uint32_t* sf_irreducible_factors_next;

  html_print( "<h2>factor</h2>" );
  html_print_p( "x", bit_size, x );
  html_br();

  html_page_push("p_gf2_sff(x)");

  p_gf2_sff_html( bit_size, x, &sf_factor_count, sf_factors, sf_factors_pow, 0 );
  ...
  {
    uint32_t* f                    = sf_factors + (i*(bit_size/32));
    uint32_t* irreducible_factors  = sf_irreducible_factors_next;
    int       irreducible_factor_count;

    char sfname[64];
    sprintf(sfname,"sf<sub>%d</sub>",i);
    html_print_p( sfname, bit_size, f );
    html_br();

    html_indent_push();
    p_gf2_berlekamp_irreducible_factors_html( bit_size, f, &irreducible_factor_count, irreducible_factors );
    if ( irreducible_factor_count == 0 )
    {
      p_gf2_mov( bit_size, irreducible_factors, f );
      irreducible_factor_count = 1;
    }
    html_indent_pop();

    sf_irreducible_factor_count[i] = irreducible_factor_count;
    sf_irreducible_factors_next   += irreducible_factor_count * (bit_size/32);
  }

  *irreducible_factor_count   = 0;
  sf_irreducible_factors_next = sf_irreducible_factors;
  for (int i=0;i<sf_factor_count;i++)
  {
    for (int j=0;j<sf_irreducible_factor_count[i];j++)
    {
      for (int k=0;k<sf_factors_pow[i];k++)
      {
        p_gf2_mov( bit_size, irreducible_factors + ((*irreducible_factor_count) * (bit_size/32)), sf_irreducible_factors_next + (j * (bit_size/32)) );
        (*irreducible_factor_count)++;
      }
    }

    sf_irreducible_factors_next += sf_irreducible_factor_count[i] * (bit_size/32);
  }
  ...
  for (int i=0;i<sf_factor_count;i++)
  {
    ...
    {
      html_print("(");
      html_print_p_bin_skiplz( bit_size, sf_irreducible_factors_next + (j * (bit_size/32)) );
      html_print(")");
    }
  ...
  for (int i=0;i<sf_factor_count;i++)
  {
    ...
    {
      html_print("(");
      html_print_p_hex_skiplz( bit_size, sf_irreducible_factors_next + (j * (bit_size/32)) );
      html_print(")");
    }
  ...
  for (int i=0;i<sf_factor_count;i++)
  {
    ...
    {
      html_print("(");
      html_print_p_poly( bit_size, sf_irreducible_factors_next + (j * (bit_size/32)) );
      html_print(")");
    }
  ...
  for (int i=0;i<sf_factor_count;i++)
  {
    ...
    {
      html_print("(");
      html_print_p_poly_input( bit_size, sf_irreducible_factors_next + (j * (bit_size/32)) );
      html_print(")");
    }
